...
<html>
    ...
    <div class="container"> 
        ...
        <?php 
            $search = $_POST['search'];
            $articles = mysqli_query($connection, "SELECT * FROM `ilmoitukset` WHERE `ilmoitus_nimi` LIKE '%$search%' OR `ilmoitus_kuvaus` LIKE '%$search%' ORDER BY `ilmoitus_id` DESC LIMIT 10");
            
            $total_count_q = mysqli_query($connection, "SELECT COUNT(`ilmoitus_id`) AS 'total_count' FROM `ilmoitukset` WHERE `ilmoitus_nimi` LIKE '%$search%' OR `ilmoitus_kuvaus` LIKE '%$search%'");
